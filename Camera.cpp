...
const spe::Ray Camera::GetPickingRay(int mouseX, int mouseY, int clientWidth, int clientHeight) const
{
	...
	// NDC 변환 및 투영 행렬 역산
	float vx = ((2.0f * mouseX) / clientWidth - 1.0f) / projMat_(0, 0);
	float vy = (-(2.0f * mouseY) / clientHeight + 1.0f) / projMat_(1, 1);
	...
	XMStoreFloat3(&dir, XMVector3Normalize(rayDirWorld));

	XMStoreFloat3(&origin, invView.r[3]);

	return spe::Ray(origin, dir);
}
