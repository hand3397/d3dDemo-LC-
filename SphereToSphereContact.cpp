...
void SphereToSphereContact::Evaluate(Manifold& manifold, const XMMATRIX& transformA, const XMMATRIX& transformB)
{
    ...
    XMStoreFloat3(&collisionInfo.pointA[0], XMLoadFloat3(&convexA.center) + (resultEPA.normal * convexA.radius));
...
void SphereToSphereContact::FindCollisionPoints(const ConvexInfo& sphereA, const ConvexInfo& sphereB, CollisionInfo& collisionInfo, ResultEPA& resultEPA, Polytope& simplexArray)
{
}
...
bool SphereToSphereContact::IsCollide(const ConvexInfo& convexA, const ConvexInfo& convexB)
{
    ...
    return ((sumRadius * sumRadius) > XMVectorGetX(XMVector3LengthSq(centerAB)));
}
...
ResultEPA SphereToSphereContact::GetResultEPA(const ConvexInfo& convexA, const ConvexInfo& convexB)
{
    ...
    // 3. 예외 처리: 두 구의 중심이 완벽하게 겹쳐있을 경우 (Distance가 0에 가까움)
    ...
    if (distance <= EPS_FLOAT)
    {
        ...
        result.dist = sumRadius; // 완전히 겹쳤으므로 반지름 합만큼 밀어내야 함
    }
    else
    {
        // 4. 충돌 법선(Normal): 중심 차이 벡터를 정규화 (A -> B 방향)
        // 이미 길이를 구했으므로 나눗셈으로 처리하는 것이 빠름
        result.normal = delta / lengthVec;

        // 5. 침투 깊이(Penetration Depth): 반지름의 합 - 현재 거리
        // 양수 값이 나올 것이며, 이 값만큼 노말 방향으로 밀어내야 함
        result.dist = sumRadius - distance;
    }
